...
public class Problem_011 {
    public static long main() {
        ...
        long greatest = -1;
        for (int position_y = 0; position_y < data.length; position_y++) {
            for (int position_x = 0; position_x < data.length - 4; position_x++) {
                long multiple = 1;
                for (int m = 0; m < 4 && multiple != 0; m++) multiple *= data[position_y][position_x + m];
                if (multiple > greatest) greatest = multiple;
            }
        }
        for (int position_x = 0; position_x < data.length; position_x++) {
            for (int position_y = 0; position_y < data.length - 4; position_y++) {
                long multiple = 1;
                for (int m = 0; m < 4 && multiple != 0; m++) multiple *= data[position_y + m][position_x];
                if (multiple > greatest) greatest = multiple;
            }
        }
        for (int position_y = 0; position_y < data.length - 4; position_y++) {
            for (int position_x = 0; position_x < data.length - 4; position_x++) {
                long multiple = 1;
                for (int m = 0; m < 4 && multiple != 0; m++) multiple *= data[position_y + m][position_y + m];
                if (multiple > greatest) greatest = multiple;
            }
        }
        for (int position_y = 0; position_y < data.length - 4; position_y++) {
            for (int position_x = data.length - 1; position_x - 4 >= 0; position_x--) {
                long multiple = 1;
                for (int m = 0; m < 4 && multiple != 0; m++) multiple *= data[position_y + m][position_x - m];
                if (multiple > greatest) greatest = multiple;
            }
        }
        return greatest;
